...
file.out <- paste("Blood ", DOI, " vs ", control, "_female", sep="")  #file.out = Substring for the output file name.
fact <- "mets" 
...
#################################################### 2. Import datasets ################################################
symlog <- function(x, base = exp(1)) {
  sign(x) * log1p(abs(x)) / log(base)
}

inv_symlog <- function(x, base = exp(1)) {
  sign(x) * (exp(abs(x) * log(base)) - 1)
}
...
datasub$color <- ifelse(datasub$Estimate > 0, 
                        ifelse(datasub$Sig == 1, "#CC0000CC", "#CC000040"), 
                        ifelse(datasub$Sig == 1, "#0066CCCC", "#0066CC40"))


datasub$Estimate_transformed <- symlog(datasub$Estimate, threshold)
...
YLIM <- c(symlog(-100, threshold), symlog(100, threshold))

# Define the ticks and labels for the y-axis
YCUTS <- symlog(c(-100, -10, -1, 0, 1, 10, 100), threshold)
YCUTS.LABS <- c("-100", "-10", "-1", "0", "1", "10", "100")
...
# Adjust Values to Fit Within YLIM
ADJ <- 0.01
datasub$RR_1[datasub$RR_1 <= YMIN] <- YMIN + (YMAX - YMIN) * ADJ
datasub$RR_2[datasub$RR_2 >= YMAX] <- YMAX - (YMAX - YMIN) * ADJ
datasub$RR_1_s[datasub$RR_1_s <= YMIN] <- YMIN + (YMAX - YMIN) * ADJ
datasub$RR_2_s[datasub$RR_2_s >= YMAX] <- YMAX - (YMAX - YMIN) * ADJ
#datasub$Estimate[datasub$Estimate <= log(YMIN)] <- log(YMIN) + (log(YMAX) - log(YMIN)) * ADJ
#datasub$Estimate[datasub$Estimate >= log(YMAX)] <- log(YMAX) - (log(YMAX) - log(YMIN)) * ADJ
...
circos.track(track.index = 1, bg.border = "white", factors = fact, ylim = YLIM, panel.fun = function(x,y){
  ...
  #circos.segments(x0=min(XLIM)-.75, y0=(YCUTS[5]), x1=max(XLIM)+0.5, y1=(YCUTS[5]), col = "gray75", lwd=lwd)
  #circos.segments(x0=min(XLIM)-.75, y0=(YCUTS[6]), x1=max(XLIM)+0.5, y1=(YCUTS[6]), col = "gray75", lwd=lwd)
...
#### Draw bars with estimates ####
  ...
  # Bars
  # 1.
circos.rect(xleft = (as.numeric(datasub$id_name_s) - .35), 
              xright = (as.numeric(datasub$id_name_s) + .35), 
              ytop = datasub$Estimate_transformed, 
              ybottom = symlog(1, base = 10), 
              col = datasub$color, 
              lwd = lwd)
  ...
  circos.rect(
    ...
    ytop = pmin(datasub$Estimate_transformed + 1.95 * sem(datasub$Estimate_transformed), max(YLIM)),
    ybottom = pmax(datasub$Estimate_transformed - 1.95 * sem(datasub$Estimate_transformed), min(YLIM)),
    col = "#262626",
    lwd = lwd
  )
